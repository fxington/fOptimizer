...
class FolderSelectionFrame(ctk.CTkFrame):
    def __init__(self, root, label: str, placeholder_text: str, tip_text: str, on_empty_text: str):
        super().__init__(root)
        ...
        self.browse_button = ctk.CTkButton(self, text="Browse", width=100, command=self.browse, fg_color="#292929", hover_color="#7e3825", border_color="#b65033", border_width=1)
    ...
    def on_empty(self):
        self.field.configure(border_color="#972222", placeholder_text=self.on_empty_text)
        self.after(2000, lambda: self.field.configure(border_color=self.border_color, placeholder_text=self.placeholder_text))
...
class ProgressWindow(ctk.CTkFrame):
    def __init__(self, root):
        ...
        self.progress_bar = ctk.CTkProgressBar(self, mode="determinate", corner_radius=5, progress_color="#b65033")
...
class OptimizationButton(ctk.CTkFrame):
    ...
    def __init__(self, root, label, description, lossless_option, remove_option, function,
        ...
        self.button = ctk.CTkButton(self, text=label, command=self.button_callback, fg_color="#292929", hover_color="#7e3825", border_color="#b65033", border_width=1)
        ...
        if level_range is not None:
            self.quality_slider = ctk.CTkSlider(self.options_frame, from_=self.level_range[0], to=self.level_range[1],
                                                number_of_steps=self.level_range[1] - self.level_range[0], command = self.on_slider_change,
            ...
            self.quality_label = ctk.CTkLabel(self.options_frame, text=f"{int(self.quality_slider.get())}", fg_color="transparent")
...
class App(ctk.CTk):
    def __init__(self, width: int = DEFAULT_WIDTH, height: int = DEFAULT_HEIGHT):
        ...
        self.input_frame = FolderSelectionFrame(self.root_scrollable,
        ...
        self.output_frame = FolderSelectionFrame(self.root_scrollable,
                                                 label="Select Output Folder", 
                                                 placeholder_text="",
                                                 tip_text="File path to output folder\nLeave this field blank to apply changes directly to the input folder.",
                                                 on_empty_text=""
                                                )
        ...
        self.progress_window.grid(row=99, column=0, padx=10, pady=(20, 0), sticky="ew", columnspan=2)

        self.description_label = DescriptionLabel(self.root_scrollable, "")
        self.description_label.grid(row=100, column=0, padx=10, pady=(10, 0), sticky="nsew") # diabolical 100 index row to anchor this to the bottom
