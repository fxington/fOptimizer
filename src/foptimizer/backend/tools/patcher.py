...
def remove_duplicate_vtfs(input_dir: Path, output_dir: Path, progress_window=None) -> bool:
    ...
    :type input_dir: Path
    :param output_dir: If specified, The absolute path of the directory to copy the duplicate VTFs to.
    ...
    try:
        if not input_dir.is_dir():
            if progress_window: progress_window.error("Remove Duplicate VTFs failed: Input folder was not a folder, or does not exist.")
            return False
        
        materials_roots = get_head_directories(input_dir=input_dir, target_dir="materials")
        if not materials_roots:
            if progress_window: progress_window.error("Remove Duplicate VTFs failed: No 'materials/' subfolders found.")
            return False
        ...
        for materials_root in materials_roots:
            ...
            for path, hash in duplicate_vtfs.items():
                if path.as_posix().lower().startswith(materials_root.as_posix().lower()):
        ...
        exception_logger(e)
        if progress_window: progress_window.error("Remove Duplicate VTFs failed with an unknown error.")
        return False